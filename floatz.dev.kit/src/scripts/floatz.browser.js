...
window.floatz.browser = (function() {
	...
	var self = {
		...
		ios: /iphone|ipod/.test(userAgent) || /ipad/.test(userAgent),
		iosVersion: /iphone|ipod/.test(userAgent) ? (userAgent.match(/.+(?:iphone\ os)[\/: ]([\d_]+)/) || [0,0])[1].toString().split('_').join('.') : (userAgent.match( /.+(?:cpu\ os)[\/: ]([\d_]+)/) || [0,0])[1].toString().split('_').join('.'),
