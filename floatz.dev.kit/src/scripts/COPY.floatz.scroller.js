		...
		$(scrollContainer).scroll(function (e) {
			...
			for (var i = 0; i < sections.length; i++) {
				if ((sections[i].orientation === Orientation.VERTICAL &&
					((sections[i].top >= scrollTop && sections[i].top <= scrollBottom) ||
					(sections[i].bottom <= scrollBottom && sections[i].bottom >= scrollTop)))
					||
					(sections[i].orientation === Orientation.HORIZONTAL &&
					((sections[i].left >= scrollLeft && sections[i].left <= scrollRight) ||
					(sections[i].right <= scrollRight && sections[i].right >= scrollLeft)))) {
