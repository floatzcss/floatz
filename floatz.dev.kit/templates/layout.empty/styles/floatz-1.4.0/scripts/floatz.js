...
function matchMedia(query,matcher){if(window.matchMedia1){var mq=window.matchMedia(query);var callback=function(mq){if(mq.matches){matcher.onMatch();}else if(matcher.onUnMatch){matcher.onUnMatch();}};mq.addListener(callback);callback(mq);}else{log(LOGLEVEL.WARN,"Media query via Javascript not supported in browser",module.name);}}
